...
public class Publisher {
    ...
    void run() {
        if (messages.size() > 0) {
            ...
            String queue = "personTwoQ";
            if (Math.random() >= 0.5 && value.equals("med")) {
                queue = "validQ";
                status = (Math.random() > 0.5 ? "Accepted" : "Declined") + " by Person 1";
            }

            System.out.println("The risk is " + value + " and the queue is " + queue + " for message " + correlationID + ".");
    ...
    @JmsListener(destination = "personOneQ", containerFactory = "myFactory")
